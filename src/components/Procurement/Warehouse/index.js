...
import {Steps , Table, Button, Message, DatePicker , Input, Spin , Row , Col} from 'antd';
...
class ConfirmPay extends PureComponent {
    ...
    // 点击提交
    handleSubmit = () => {
        let {inboundTiStr,signer} = this.state;
        if(!inboundTiStr || !signer){
            Message.warning('请完善信息后提交');
            return false;
        }
        this.props.handleSubmit(inboundTiStr,signer)
    }
    ...
    render() {
        ...
        if(!this.props.data.resultInfo){
            ...
            console.log(this.props.data.resultInfo)
            ...
            return (
                <div>
                    <Steps progressDot current={2} className={styles.steps}>
                        <Step title="已下单" />
                        <Step title="待付款" />
                        <Step title="待入库" />
                        <Step title="已完成" />
